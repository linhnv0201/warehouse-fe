...
export default function Customers() {
  ...
  const fetchCustomers = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:8080/warehouse/customers", {
  ...
  const handleAddCustomer = async () => {
    try {
      await axios.post("http://localhost:8080/warehouse/customers", form, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleEditCustomer = async () => {
    try {
      await axios.put(`http://localhost:8080/warehouse/customers/${currentEdit.id}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteCustomer = async () => {
    try {
      await axios.delete(`http://localhost:8080/warehouse/customers/${currentDelete.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
